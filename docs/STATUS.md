...
| **المتابعة والذكاء** tracking & AI | Reports now cover **all three logged pillars** plus **challenges**. **الملف الشخصي, CONNECT and «ابني خطتي» are built end to end** — key in the keystore, one call through one file, the plan shown on a sheet. Not yet *applied* to the day: propose vs overwrite is the user's open question. |
...
- **الإعدادات is a menu** — nine sections, each its own page; الذكاء الاصطناعي
  is the ninth, since 13 September
...
- **A switch for the task alarms** in الإشعارات, on by default; it never
  touches the adhan
- **A sound per task** — nineteen channels, nineteen tones, none shared.
  Since 13 September **eleven are the user's own recordings** (water, walk,
  tasbeeh, wird, the three athkar, qiyam, knowledge, phone, iqama), installed
  by `tool/install_alert_recordings.py`; the other eight are still
  synthesised by `tool/make_alert_sounds.py` — مكالمات a landline's rasp,
  تمارين a whistle, وجبة a spoon on a glass, الميزانية coins. 2.5 MB for all
  nineteen, under the five adhans. The eight are still **measured** apart by
  `alert_sound_character_test`; the eleven are his by choice.
...
- **One network call, through one file.** `INTERNET` is in the manifest since
  13 September; `lib/features/ai/ai_http_client.dart` is the only file that
  may open a socket, and the only hosts anywhere under `lib/` are the three
  AI services. Opened only by CONNECT (free — lists models) and «ابني خطتي».
- **Eleven of the nineteen task tones are the user's own recordings**, each
  on a fresh channel; the eight he did not send are byte-identical. The iqama
...
- **The recitations' licences cannot be individually verified.** The Wikimedia
  ...
  thing to revisit.
- **The AI call is built but has never been made for real.** No key has been
  entered; CONNECT and «ابني خطتي» have run only against fakes and a loopback
  server. The three default model names — `claude-haiku-4-5`, `gpt-4o-mini`,
  `gemini-2.5-flash` — are unverified against the live services; CONNECT's
  «اختار» shows what is really there.
- **The user's recordings vary widely in loudness** (peaks 0.33–1.59) and
  none has been heard on the phone. Normalising them is one flag in
  `tool/install_alert_recordings.py`, held back until he listens.
- **The plan is proposed, not applied.** Mapping `PlanDocument` onto the
  `planDay` pipeline (spec §4) waits on the propose-or-overwrite answer.
...
| Guard | What it stops |
|---|---|
| `no_network_test` | A second file opening a socket; a URL literal outside `ai_provider.dart`; a host that is not one of the three AI services; a network package as a dependency |
...
| `alert_sound_character_test` | Two synthesised tones that *measure* alike — it reads the PCM, not the filename. For the user's eleven MP3s: two that are the same file, or one too short to be a sound |
| `notification_channels_test` | One of the user's eleven recordings shipping under the channel id the old tone was on — Android would keep playing the old tone |
...
tool/make_alert_sounds.py     the eight synthesised tones; each builder says
                              what it should sound like. Builds all nineteen,
                              writes eight — the RNG sequence must not move
tool/install_alert_recordings.py
                              the user's eleven recordings → res/raw, and the
...
docs/superpowers/handoffs/2026-09-13-the-night-of-four-things.md
                              the recordings, the prayers line, the five
                              words, CONNECT — and the two decisions to reverse
```
